...
export default function CryptoFailuresPage() {
  ...
  async function handleScan() {
    ...
    try {
      const data = await scanUrl(url, "crypto_failures");
      setResult(JSON.stringify(data, null, 2));
    } catch (error) {
      console.error("Cryptographic Failures scan error:", error);
      setResult("Error running Cryptographic Failures scan.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-4">Cryptographic Failures</h1>
      <div className="mb-4">
      ...
      >
        {loading ? "Scanning..." : "Run Crypto Failures Scan"}
      </button>
