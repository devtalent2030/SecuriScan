...
export default function CsrfPage() {
  ...
  async function handleScan() {
    ...
    try {
      // "csrf" => your backend's endpoint /check_csrf
      const data = await scanUrl(url, "csrf");
      setResult(JSON.stringify(data, null, 2));
    } catch (error) {
      console.error("CSRF scan error:", error);
      setResult("Error running CSRF scan.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-4">CSRF (Cross-Site Request Forgery)</h1>
      <div className="mb-4">
