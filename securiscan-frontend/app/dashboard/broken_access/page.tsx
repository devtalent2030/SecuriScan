...
export default function BrokenAccessPage() {
  ...
  async function handleScan() {
    ...
    try {
      // "broken_access" => your backend's endpoint /check_access_control
      const data = await scanUrl(url, "access_control");
      setResult(JSON.stringify(data, null, 2));
    } catch (error) {
      console.error("Broken Access Control error:", error);
      setResult("Error running Broken Access Control scan.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-4">Broken Access Control</h1>
      <div className="mb-4">
