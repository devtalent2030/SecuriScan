...
import { ChevronDown, ChevronUp, Lock, X, Minimize2, Copy } from "lucide-react";
...
export default function AuthFailuresPage() {
  ...
  const copyExampleUrl = () => {
    navigator.clipboard.writeText("http://testphp.vulnweb.com/login.php");
  ...
  const downloadPdf = () => {
    ...
    // Add title
    doc.setFontSize(16);
    doc.text("Authentication Failures Report", 10, yPos);
    yPos += 10;
    ...
    yPos += 5;
    doc.setFontSize(10);
    doc.text(`Scan ID: AUTH-${new Date().getTime()}`, 15, yPos);
    yPos += 5;
    ...
    // Vulnerabilities
    doc.setFontSize(12);
    doc.text("2. Detected Authentication Vulnerabilities", 10, yPos);
    yPos += 5;
    doc.setFontSize(10);
    if (result.error) {
      ...
      doc.text("No authentication vulnerabilities detected.", 15, yPos);
    }

    doc.save(`Auth-Failures-Report-${new Date().getTime()}.pdf`);
  };
