...
import { ChevronDown, ChevronUp, Code, X, Minimize2, Copy } from "lucide-react";
...
export default function XssPage() {
  ...
  const copyExampleUrl = () => {
    navigator.clipboard.writeText("http://testphp.vulnweb.com/search.php?test=query");
  ...
  const downloadPdf = () => {
    ...
    yPos += 5;
    doc.setFontSize(10);
    doc.text(`Scan ID: XSS-${new Date().getTime()}`, 15, yPos);
    yPos += 5;
    ...
    // Vulnerabilities
    doc.setFontSize(12);
    doc.text("2. Detected XSS Vulnerabilities", 10, yPos);
    yPos += 5;
    doc.setFontSize(10);
    if (result.error) {
      ...
      doc.text("No XSS vulnerabilities detected.", 15, yPos);
    }

    doc.save(`XSS-Report-${new Date().getTime()}.pdf`);
  };
