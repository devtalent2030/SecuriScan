...
export default function DirectoryEnumPage() {
  ...
  async function handleScan() {
    ...
    try {
      // "directory_enum" => your backend's endpoint /check_dirs
      const data = await scanUrl(url, "dirs");
      setResult(JSON.stringify(data, null, 2));
    } catch (error) {
      console.error("Directory enumeration error:", error);
      setResult("Error running directory enumeration.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-4">Directory Enumeration</h1>
      <div className="mb-4">
      ...
      >
        {loading ? "Scanning..." : "Run Directory Scan"}
      </button>
