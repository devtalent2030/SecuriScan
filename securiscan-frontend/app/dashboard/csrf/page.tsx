...
import { ChevronDown, ChevronUp, ShieldOff, X, Minimize2, Copy } from "lucide-react";
...
export default function CsrfPage() {
  ...
  const copyExampleUrl = () => {
    navigator.clipboard.writeText("http://testasp.vulnweb.com/login.asp");
  ...
  const downloadPdf = () => {
    ...
    yPos += 5;
    doc.setFontSize(10);
    doc.text(`Scan ID: CSRF-${new Date().getTime()}`, 15, yPos);
    yPos += 5;
    ...
    // Vulnerabilities
    doc.setFontSize(12);
    doc.text("2. Detected CSRF Vulnerabilities", 10, yPos);
    yPos += 5;
    doc.setFontSize(10);
    if (result.error) {
      doc.text(result.error, 15, yPos);
    } else if (result.vulnerabilities && result.vulnerabilities.length > 0) {
      doc.text(`Found ${result.vulnerabilities.length} potential CSRF vulnerability(s).`, 15, yPos);
      yPos += 5;
      ...
      doc.text("No CSRF vulnerabilities detected.", 15, yPos);
    }

    doc.save(`CSRF-Report-${new Date().getTime()}.pdf`);
  };
