...
export default function DependencyScannerPage() {
  ...
  const copyExampleUrl = () => {
    navigator.clipboard.writeText("https://getbootstrap.com");
  ...
  const downloadPdf = () => {
    ...
    // Add title
    doc.setFontSize(16);
    doc.text("Dependency Scanner Report", 10, yPos);
    yPos += 10;
    ...
    yPos += 5;
    doc.setFontSize(10);
    doc.text(`Scan ID: DEP-${new Date().getTime()}`, 15, yPos);
    yPos += 5;
    ...
    // Vulnerabilities
    doc.setFontSize(12);
    doc.text("2. Detected Dependency Vulnerabilities", 10, yPos);
    yPos += 5;
    doc.setFontSize(10);
    if (result.error) {
      ...
      doc.text("No dependency vulnerabilities detected.", 15, yPos);
    }

    doc.save(`Dependency-Report-${new Date().getTime()}.pdf`);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 bg-gradient-to-br from-gray-950 via-indigo-950 to-purple-950 rounded-2xl shadow-2xl animate-gradient text-white">
      ...
      <p className="text-gray-300 text-sm mb-8 leading-relaxed">
        Activate this tool to detect <strong>vulnerable JavaScript dependencies</strong>—outdated libraries that may put your site at risk. Paste a URL to test.
      </p>
