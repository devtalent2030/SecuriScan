...
export default function CommandInjectionPage() {
  ...
  async function handleScan() {
    ...
    try {
      // "cmd_injection" => your backend's endpoint /check_cmd_injection
      const data = await scanUrl(url, "cmd_injection");
      setResult(JSON.stringify(data, null, 2));
    } catch (error) {
      console.error("Command injection error:", error);
      setResult("Error running command injection scan.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-4">Command Injection</h1>
      <div className="mb-4">
