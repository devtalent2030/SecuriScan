...
import { ChevronDown, ChevronUp, Terminal, X, Minimize2, Copy } from "lucide-react";
...
export default function CommandInjectionPage() {
  ...
  const copyExampleUrl = () => {
    navigator.clipboard.writeText("http://testasp.vulnweb.com/showthread.asp?id=1");
  ...
  const downloadPdf = () => {
    ...
    // Add title
    doc.setFontSize(16);
    doc.text("Command Injection Report", 10, yPos);
    yPos += 10;
    ...
    yPos += 5;
    doc.setFontSize(10);
    doc.text(`Scan ID: CMD-${new Date().getTime()}`, 15, yPos);
    yPos += 5;
    ...
    // Vulnerabilities
    doc.setFontSize(12);
    doc.text("2. Detected Command Injection Vulnerabilities", 10, yPos);
    yPos += 5;
    doc.setFontSize(10);
    if (result.error) {
      ...
      doc.text("No command injection vulnerabilities detected.", 15, yPos);
    }

    doc.save(`Command-Injection-Report-${new Date().getTime()}.pdf`);
  };
