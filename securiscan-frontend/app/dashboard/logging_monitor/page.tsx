...
export default function LoggingMonitorPage() {
  ...
  async function handleScan() {
    ...
    try {
      const data = await scanUrl(url, "logging_monitor");
      setResult(JSON.stringify(data, null, 2));
    } catch (error) {
      console.error("Logging & Monitoring scan error:", error);
      setResult("Error running Logging & Monitoring scan.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-4">Logging & Monitoring Failures</h1>
      <div className="mb-4">
